          ...
          <URadioGroup v-model="value" :items="items" class="[&>fieldset]:space-y-3 mt-6" indicator="hidden" variant="list">
            <template #label="{ item, modelValue }">
              <div class="flex items-center justify-between w-full cursor-pointer transition">
                <span class="font-inter text-left !font-normal md:font-medium text-brand-light-primary dark:text-white dark:!bg-transparent leading-[120%]" :class="{ 'opacity-50': modelValue !== ((item as any).value ?? item) }">{{ (item as any).label ?? item }}</span>
