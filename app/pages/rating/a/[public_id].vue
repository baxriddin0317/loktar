...
onMounted(async () => {
  profile.value = await services.getAgencyProfile(publicId.value)
  const img = await services.getAgencyLogo(publicId.value)
  logoUrl.value = img.imageUrl
...
watch(
  ...
  async (auth) => {
    if (auth === true) {
      try {
        contacts.value = await services.getAgencyContacts(publicId.value)
...
<template>
  <div class="container mx-auto p-4 space-y-4">
    ...
    <div class="flex items-center gap-4">
      <img v-if="logoUrl" :src="logoUrl" alt="Логотип" width="96" height="96" class="rounded" />
      <div>
        <h1 class="text-2xl font-semibold">{{ profile?.name ?? 'Анонимное агентство' }}</h1>
